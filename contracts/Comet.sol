...
contract Comet is CometMainInterface {
    ...
    function isBorrowCollateralized(
        ...
        for (uint8 i = 0; i < numAssets; ) {
            if (isInAsset(assetsIn, i)) {
                ...
                uint borrowValue = mulPrice(
                    userCollateral[account][asset.asset].borrowBalance,
                    getPrice(asset.priceFeed),
                    asset.scale
                );
                liquidity -= signed256(borrowValue);
            }
            unchecked {
                i++;
            }
    ...
    function borrow(address asset, uint128 amount) external override {
    ...
    function borrowCollateral(
        ...
        uint128 srcBorrowBalance = userCollateral[src][asset].borrowBalance;
        uint128 srcBorrowBalanceNew = srcBorrowBalance + amount;

        totalsCollateral[asset].totalBorrowAsset += amount;
        userCollateral[src][asset].borrowBalance = srcBorrowBalanceNew;
